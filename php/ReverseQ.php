...
$sql ="SELECT `recaudo`.`idRecaudo`, `recaudo`.`idconsumo_factura`, `recaudo`.`Valor Recaudado`, `recaudo`.`tipo`, `recaudo`.`Fecha`, `consumo_factura`.`valor_total`, `consumo_factura`.`id_Usuario`, `registro_usuario`.`Nombre_usuario`, `registro_usuario`.`Apellido`, `registro_usuario`.`Indentificacion` FROM `recaudo` JOIN `consumo_factura` ON `recaudo`.`idconsumo_factura` = `consumo_factura`.`idconsumo_factura` JOIN `registro_usuario` ON `consumo_factura`.`id_Usuario` = `registro_usuario`.`id_Usuario` WHERE `recaudo`.`tipo`='Pago Total' OR `recaudo`.`tipo`='Abono a Factura' ORDER BY `recaudo`.`idRecaudo` ASC";
...
while($row=mysqli_fetch_array($query)){
    ...
    $subdata[]=$row[8]; //age  
    $subdata[]=$row[9]; //age  
    $subdata[]='<button type="button" id="getEdit" class="btn btn-primary btn-xs" data-toggle="modal" title="Reversar Pago" data-idconsumo_factura="'.$row[0].'"><i class="fas fa-history">&nbsp;</i></button>';
    $datas[]=$subdata;
}
